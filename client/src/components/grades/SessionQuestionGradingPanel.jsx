    ...
    <Box>
      ...
      <Dialog open={aiGradingReportOpen} onClose={() => setAiGradingReportOpen(false)} fullWidth maxWidth="md">
        ...
        <DialogContent dividers>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}><Typography variant="subtitle2">{t('grades.aiGrading.log')}</Typography><Button size="small" color="error" onClick={async () => { await apiClient.delete(`/ai/courses/${courseId}/sessions/${sessionId}/ai-grading-log`); setAiGradingJob((current) => current ? { ...current, sessionLog: null } : current); }}>{t('grades.aiGrading.clearLog')}</Button></Box>
          {(aiGradingJob?.sessionLog?.runs || [{ startedAt: aiGradingJob?.createdAt, entries: aiGradingJob?.log || [] }]).map((run, runIndex) => <Box key={run.jobId || runIndex} sx={{ mb: 2 }}><Typography variant="caption" color="text.secondary">{t('grades.aiGrading.logRunAt', { timestamp: run.startedAt ? new Date(run.startedAt).toLocaleString() : '' })}</Typography>{(run.entries || []).map((entry, index) => <Paper key={index} variant="outlined" sx={{ p: 1, mb: 1 }}><Typography variant="subtitle2">{entry.question} · {entry.student}</Typography>{entry.status !== 'skipped' ? <Typography variant="body2">{t('grades.aiGrading.assignedGrade', { points: entry.points, outOf: entry.outOf })}</Typography> : null}{entry.note ? <Typography variant="body2" sx={{ whiteSpace: 'pre-wrap', mt: 0.5 }}>{entry.note}</Typography> : null}{entry.justification ? <Typography variant="body2" sx={{ whiteSpace: 'pre-wrap', mt: 0.5 }}><strong>{t('grades.aiGrading.justification')}:</strong> {entry.justification}</Typography> : null}{entry.feedback ? <Typography variant="body2" sx={{ whiteSpace: 'pre-wrap', mt: 0.5 }}>{entry.feedback}</Typography> : null}</Paper>)}</Box>)}
        </DialogContent>

...
| `/api/v1/activity-codes` and session activity-share paths | Instructor code management, signed-in redemption, and session-scoped quiz/live participation |
...
Course `POST /api/v1/courses` and `PATCH /api/v1/courses/:id` accept `allowSharedActivities: boolean`, defaulting to `false`. When disabled, code issuance and redemption are denied; turning it off revokes existing codes and grants for the course. Existing course-member access is unaffected. `GET /api/v1/sessions/:id/activity-share` returns `{ enabled, code, expiresAt }` to a course instructor or admin. `POST /api/v1/sessions/:id/activity-share` issues or rotates a code and returns `{ code, expiresAt }`; an optional `expiresAt` must be within the next year, with 30 days as the default. The current code is also available to course instructors on later status reads while sharing is enabled. Older hash-only codes cannot be recovered and must be regenerated to display them. `DELETE /api/v1/sessions/:id/activity-share` disables sharing and revokes existing grants. Practice quizzes, student-created sessions, inactive courses, and sessions with existing grades or individual quiz extensions cannot issue codes. Once a code has been issued, individual quiz extensions remain unavailable even if sharing is disabled. A session that has ever been shared remains ungraded after sharing is disabled. Its grades, manual marks, AI grading, and course gradebook entries are unavailable.

`POST /api/v1/activity-codes/redeem` accepts `{ code }` from an authenticated user and returns `{ sessionId, courseId, name, quiz, anonymous }`. New codes are `S-` plus ten characters from an unambiguous 32-character alphabet (50 bits of code space); previously issued 40-character hexadecimal codes remain redeemable. Unknown, disabled, and unavailable codes receive the same 404 response. An expired code cannot admit a new account, but an account with a still-valid grant may enter it again. Redemption is rate limited. A successful redemption records a session-scoped grant and locks the session's anonymity mode; it does not enroll the user in the course or create a grade. Rotating a code preserves existing grants; disabling sharing revokes them, including after a later reissue.

A valid grant authorizes only that session’s participant metadata, quiz read/autosave/submit, live read/join/respond, telemetry, and own-response review. It does not authorize course rosters, course chat, grades, question libraries, or other sessions. The dashboard routes prefixed activity codes to redemption and ordinary enrollment codes to `/courses/enroll`. An outside participant uses `/activity/:courseId/session/:sessionId/quiz` or `/live`; the route ID is display/navigation context and the server checks the grant by session ID. Outside users do not get session chat. Live join-code settings still apply after grant redemption. WebSocket question, visibility, attempt, status, join-code, and response events resolve current grant holders; anonymous recipient mapping remains server-only.

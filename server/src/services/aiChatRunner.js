...
const MAX_CODE_RECOVERY_ATTEMPTS = 1;
...
export function latestUserRequestsCode(messages) {
  const content = String([...(messages || [])].reverse().find((message) => message.role === 'user')?.content || '');
  return /\b(?:provide|show|give|send|share|include|write|display)\b[\s\S]{0,100}\b(?:source\s+code|code)\b/i.test(content)
    || /\b(?:python|javascript|typescript|java|c\+\+|r|matlab)\s+code\b/i.test(content);
}

export function responseContainsRequestedCode(response) {
  if (/```[^\n]*\n[\s\S]+?```/.test(String(response?.content || ''))) return true;
  return (Array.isArray(response?.artifacts) ? response.artifacts : []).some((artifact) => {
    ...
    return /\.(?:py|js|jsx|ts|tsx|java|c|cc|cpp|h|hpp|r|m|sh|sql|ipynb|txt)$/.test(filename)
      || mimeType.startsWith('text/');
  });
}

function missingCodeRecoveryMessage() {
  return {
    role: 'system',
    content: 'The user explicitly requested source code, but the preceding response included neither a code block nor a code-file artifact. Do not claim that code was provided or attached. Return the complete requested code now in a fenced code block with the appropriate language tag, or as an actual code-file artifact.',
  };
}
  ...
  try {
    ...
    for (let round = 0; round < maxToolRounds; round += 1) {
      ...
      if (response.toolCalls.length === 0) {
        if (latestUserRequestsCode(messages) && !responseContainsRequestedCode(response)) {
          if (codeRecoveryAttempts < MAX_CODE_RECOVERY_ATTEMPTS && round + 1 < maxToolRounds) {
            codeRecoveryAttempts += 1;
            providerMessages.push({ role: 'assistant', content: response.content || '' });
            providerMessages.push(missingCodeRecoveryMessage());
            continue;
          }
          throw new Error('AI backend did not include the requested source code in its response');
        }
        if (creationStillIncomplete(creationGoals, creationProgress)) {
